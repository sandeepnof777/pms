...
#filterListNew ul li:hover > .created_badge {  width: 58px; }
...
#filterListNew ul li:hover > .bt_badge {  width: 88px; }
#filterListNew ul li{
    display:inline;
                ...
                <div id="filterListNew">
                        ...
                        <?php } }?>
                            ...
                            if($this->uri->segment(5) == 'won' || $this->uri->segment(6) == 'won'){echo 'Won ';}
                                if($created_preset && $created_preset !='custom'){
                                    ...
                                    echo $this->session->userdata('accFilterFrom') .' - '. $this->session->userdata('accFilterTo'); ?>
                                <?php }?>    
                            </strong>
                    ...
                    <?php }?> 
                        ...
                        if($this->session->userdata('pAccStatsFilterUser')){
                            if(count($this->session->userdata('pAccStatsFilterUser'))<2){
                                $users = $this->session->userdata('pAccStatsFilterUserNames');
                                $show_users = $users[0];
                            }else{
                                $tiptip_names = '';
                                foreach($this->session->userdata('pAccStatsFilterUserNames') as $name){
                                    $tiptip_names .= $name.'<br/>';
                                }
                                
                                $show_users = 'Users <span class="tiptip" title="'.$tiptip_names.'">['.count($this->session->userdata('pAccStatsFilterUser')).']</span>';
                            }
                            ?>
                            ...
                            <strong><?php echo $show_users ?></strong></li>

                        <?php } ?>
                    
                    <?php
                
                    if($this->session->userdata('pAccStatsFilterStatusNameShow')){?>
                        ...
                        <strong><?php echo $this->session->userdata('pAccStatsFilterStatusNameShow'); ?></strong></li> 
                    
                <?php  }else if($this->session->userdata('pAccStatsFilterStatusName')){?>
                        <li><span class="badge-icon"><i class="fa fa-fw fa-bar-chart"></i></span>
                        <span class="rightbox status_badge">
                                    <span class="badge_content"><strong>Status:</strong></span>
                                </span>
                        <strong><?php echo ucfirst($this->session->userdata('pAccStatsFilterStatusName')); ?></strong></li> 
...
//        updateBranchUsers();
                ...
                $count = 0;
                if($this->session->userdata('pAccStatsFilterUser')){
                    $count = count($this->session->userdata('pAccStatsFilterUser'));
                }
                ?>
                ...
                var statusFrom  = '<?= $this->session->userdata('pAccStatsFilterFrom');?>';
                if(statusFrom!=''){
                ...
                var statusName  = '<?= $this->session->userdata('pAccStatsFilterStatusName');?>';
                var statusNameShow  = '<?= $this->session->userdata('pAccStatsFilterStatusNameShow');?>';

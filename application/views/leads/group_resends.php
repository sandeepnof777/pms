...
<div id="content" class="clearfix">
    <p>
        <span style="font-size: 15px;margin-top: 10px;position: absolute;"><strong>Lead Campaigns</strong></span>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        oTable = $('.estimatePricingTable').on('error.dt', function (e, settings, techNote, message) {
            ...
            .DataTable({
                ...
                "ajax": "<?php echo site_url('leads/group_resends_data/'); ?>",
                "columns": [
                    {
                        width: '10px',
                        searchable: false,
        ...
        $(document).on('click', "#groupCampaignDelete", function() {
            ...
            swal({
                ...
                if (isConfirm) {
                    ...
                    $.ajax({
                        url: '/ajax/group_delete_lead_campaigns',
                        type: "POST",
                        dataType: "json",
                        data: {
        ...
        swal({
            ...
            if (isConfirm) {
                ...
                $.ajax({
                    url: '/ajax/leadGroupResendUnopened',
                    type: "POST",
                    dataType: "json",
                    data: {
    ...
    $("#resend-proposals-status").dialog({
                ...
                "Resend": {
                    ...
                    click: function () {
                        ...
                        $.ajax({
                            ...
                            url: "<?php echo site_url('ajax/leadResendUnopened') ?>?" + Math.floor((Math.random() * 100000) + 1),
                            dataType: "JSON"
                        })
                            .success(function (data) {
        ...
        $(document).on('click', ".show_email_content", function () {
            ...
            var resend_id = $(this).attr('data-resend-id');
            $.ajax({
                        url: '/ajax/get_lead_resend_counts_details/',
                        type: "POST",
                        dataType: "json",
                        data: {
                            "resend_id": resend_id,
                        ...
                        success: function (data) {
                            if (data.success) {
                                ...
                                var lead_filters = JSON.parse(data.filters);
                            ...
                            if (data.success) {
                                if(data.total_resending>0){
                                    ...
                                    if(unclicked=='1'){
                                        ...
                                        $('#resend-proposals-title').html('<i class="fa fa-fw fa-share-square-o"></i> Resend Unclicked Lead Emails');
                                    }else{
                                        $('#resend-proposals-title').html('<i class="fa fa-fw fa-share-square"></i> Resend Unopened Lead Emails');
                                    }
...
function check_highlighted_row(){
    if(localStorage.getItem("lec_last_active_row")){
        var row_num =localStorage.getItem("lec_last_active_row");
...
$(document).on('click', '.estimatePricingTable tbody td a, .estimatePricingTable tbody td span',  function() {
    ...
    if(hasLocalStorage){
        localStorage.setItem("lec_last_active_row", row_num);
    }
...
$(document).on('click', '.javascript_loaded',  function(e) {
    $('tr.selectedRow').removeClass('selectedRow');
    if(hasLocalStorage){
        localStorage.setItem("lec_last_active_row", '');
    }
});
   ...
   $(document).on('click', ".edit_resend_name", function () {
    ...
    swal({
        ...
        onOpen: function () {
            ...
            $.ajax({
                        url: '/ajax/update_lead_resend_name',
                        type: "POST",
                        dataType: "json",
                        data: {
                            "resendId": resendId,

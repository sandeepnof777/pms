...
<table id="superCompanyAccountsTable" cellpadding="0" cellspacing="0" border="0" class="display">
                    <thead>
                    <tr>
                        <td></td>
                        <td>ID#</td>
                    ...
                    <tr>
                        ...
                        <td>
                            <a href="<?php echo site_url('account/super_edit_user/' . $account->getAccountid()) ?>" class="tiptip btn-edit">Edit User</a>
                            <a class="btn-reassign reassign-clients tiptip" title="Reassign Contacts, Proposals &amp; Accounts" href="#" rel="<?php echo $account->getAccountId() ?>">Reassign Contacts, Proposals &amp; Accounts</a>
                            <a class="btn-history tiptip" href="<?php echo site_url('history/super_user/' . $account->getAccountId()) ?>" title="View user history">View User History</a>
                            <a class="btn-delete tiptip" title="Delete" href="<?php echo site_url('account/delete_super_user/' . $account->getAccountId().'/'.$account->getCompany()->getCompanyId()) ?>">Delete User</a>
                        </td>
                    </tr>
                        <?php
                    }
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $('#superCompanyAccountsTable').dataTable({
            "bProcessing": true,
            "aoColumns": [
                null, // 0 Checkbox
                null, // 1 ID
                ...
                {"bVisible": false}, // 5 Last Login Timestamp
                {"iDataSort": 5}, // 6 Last Login
                {"bVisible": false}, // 7 Expiry Timestamp
                {"iDataSort": 7}, // 8 Expiry
                {"sWidth": '145px'}// 9 Actions
            ],
            "bJQueryUI": true,
            "bAutoWidth": false,
        ...
        var userList = {};
            <?php
                ...
                foreach ($accounts as $account) {
                    ...
                    ?>
                    userList[<?php echo $account->getAccountId() ?>] = [
                <?php
                $comUsers = $account->getCompany()->getActiveSortedAccounts();
                $k = 0;
                foreach ($comUsers as $account) {
                    $k++;
                    ?>[<?php echo $account->getAccountId() ?>, '<?php echo addslashes($account->getFullName()) ?>']<?php
                echo ($k < count($comUsers)) ? ",\n" : "\n";
            }
            ?>
        ];
        <?php } ?>
        ...
        $('.reassign-clients').live('click', function () {
            ...
            $("#uniform-reassignUser span").html('');
            $("#reassignUser").html('');
            var showUsers = userList[reassignFromId];
            for (i in showUsers) {
...
<div id="reassign-clients" title="Reassign Contacts">
    <div style="position: relative;">
        <form action="<?php echo site_url('account/super_reassign_clients') ?>" method="post" id="reassign-form">

        ...
        <table>
            <tr>
                ...
                <td style="font-size: 16px;">
                    ...
                    <p><a href="<?php echo $proposal->getAuditReportUrl(true) ?>">View your Custom Site Inspection/Audit Report</a></p>
                </td>
            </tr>
        </table>
...
foreach ($services as $service) {
    ...
    if(!$proposal->hasSnow()){
        ?>
        <div style="position: relative;page-break-inside: avoid" id="item_<?php echo $k ?>" >
            <div class="item-content<?php echo ($service->isNoPrice()) ? ' no-price' : ''; ?>">
                <h2><?php echo $service->getServiceName() ?></h2>
                <?php 
                    if ($service->isOptional()) {
                ?>
                    <div style="height:10px;position: absolute;right: 35px;top:10px;font-size: 12px;background-color: #6c757d!important;" class="badge bg-secondary">Optional Service</div>
                <?php }?>
                <div class="item-content-texts">
                    <?php echo nl2br($services_texts[$service->getServiceId()]); ?>
                </div>
                <?php if (!$lumpSum  && !$service->isNoPrice()) { ?>
                    <span style="padding-left: 40px; margin-top: 0 !important;">Total Price for this item: <?php  echo $service->getFormattedPrice();  ?></span>
                <?php } 
...
<!--<p>&nbsp;</p>-->
<p>Please find the following breakdown of all services we have provided in this proposal. This proposal originated on <?php echo date('F d, Y', $proposal->getCreated(false)) ?>.
    <?php
...
<?php if (trim($proposal->getClient()->getCompany()->getContractCopy())) { ?>
    ...
    <h2 style="margin-top: 2px; margin-top: 5px;">Authorization to Proceed & Contract</h2>
        ...
        <tr>
            ...
            <td valign="top" width="45%">
                ...
                <?php } else { ?>
                    ...
                    <br>
                    <a href="mailto:<?php echo $proposal->getOwner()->getEmail() ?>"><?php echo $proposal->getOwner()->getEmail() ?></a><br>
                    C: <?php echo $proposal->getOwner()->getCellPhone() ?>
                    <br>
...
if (count($attachments) || count($proposal_attachments)) {
    ...
    if (count($attachments)) {
        ...
        foreach ($attachments as $attachment) {
            ...
            ?>
            <p style="margin: 0; padding: 5px 0 5px 0;"><a href="<?php echo (float) str_replace(' ', '%20', $url) ?>"><?php echo $attachment->getFileName() ?></a></p>
            <?php
        }
    }
    if (count($proposal_attachments)) {
        ...
        foreach ($proposal_attachments as $attachment) {
            ...
            ?>
            <h3 style="margin: 0; padding: 5px 0 5px 0;"><a href="<?php echo (float) str_replace(' ', '%20', $url) ?>"><?php echo $attachment->getFileName() ?></a></h3>
            <?php
        }
    }
}

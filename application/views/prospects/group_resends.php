...
<div id="content" class="clearfix">
    <p>
        <span style="font-size: 15px;margin-top: 10px;position: absolute;"><strong>Prospect Campaigns</strong></span>
        
        <a style="margin-bottom: 10px;" href="<?php echo site_url('prospects'); ?>" class="btn right">
...
<div id="resend-proposals" title="Confirmation">
    ...
    <!-- <p style="margin-bottom: 15px;" class="new_resend_name_span"><label style="padding-right: 29px;font-weight:bold">Campaign Name:</label>
    ...
    $(document).ready(function () {
        ...
        oTable = $('.estimatePricingTable').on('error.dt', function (e, settings, techNote, message) {
            ...
            .DataTable({
                ...
                "ajax": "<?php echo site_url('prospects/group_resends_data/'); ?>",
                "columns": [
                    {
                        width: '10px',
                        searchable: false,
        ...
        $(document).on('click', "#groupCampaignDelete", function() {
            ...
            swal({
                ...
                if (isConfirm) {
                    ...
                    $.ajax({
                        url: '/ajax/group_delete_prospect_campaigns',
                        type: "POST",
                        dataType: "json",
                        data: {
        ...
        swal({
            ...
            if (isConfirm) {
                ...
                $.ajax({
                    url: '/ajax/prospectGroupResendUnopened',
                    type: "POST",
                    dataType: "json",
                    data: {
                ...
                "Resend": {
                    ...
                    click: function () {
                        ...
                        $.ajax({
                            ...
                            url: "<?php echo site_url('ajax/prospectResendUnopened') ?>?" + Math.floor((Math.random() * 100000) + 1),
                            dataType: "JSON"
                        })
                            .success(function (data) {
                        ...
                        $(this).dialog('close');
                        $("#resendProposalsStatus").html('Sending Prospect mails...<img src="/static/loading.gif" />');
                        $("#resend-proposals-status").dialog('open');
                        //swal("Success", "Prospect Emails Resent");
                    }
                },
                Cancel: function () {
                    $(this).dialog("close");
                            ...
                            if (data.success) {
                                if(data.total_resending>0){
                                    ...
                                    if(unclicked=='1'){
                                        ...
                                        $('#resend-proposals-title').html('<i class="fa fa-fw fa-share-square-o"></i> Resend Unclicked Prospect Emails');
                                    }else{
                                        $('#resend-proposals-title').html('<i class="fa fa-fw fa-share-square"></i> Resend Unopened Prospect Emails');
                                    }
        ...
        $(document).on('click', ".delete_campaign", function () {
            ...
            swal({
                ...
                if (isConfirm) {
                    ...
                    $.ajax({
                        url: '/ajax/delete_prospect_campaign/',
                        type: "POST",
                        dataType: "json",
                        data: {
                            "resend_id": resend_id,
...
function check_highlighted_row(){
    if(localStorage.getItem("psc_last_active_row")){
        var row_num =localStorage.getItem("psc_last_active_row");
...
$(document).on('click', '.estimatePricingTable tbody td a, .estimatePricingTable tbody td span',  function() {
    ...
    if(hasLocalStorage){
        localStorage.setItem("psc_last_active_row", row_num);
    }
...
$(document).on('click', '.javascript_loaded',  function(e) {
    $('tr.selectedRow').removeClass('selectedRow');
    if(hasLocalStorage){
        localStorage.setItem("psc_last_active_row", '');
    }
});
   ...
   $(document).on('click', ".edit_resend_name", function () {
    ...
    swal({
        ...
        onOpen: function () {
            ...
            $.ajax({
                        url: '/ajax/update_prospect_resend_name',
                        type: "POST",
                        dataType: "json",
                        data: {
                            "resendId": resendId,

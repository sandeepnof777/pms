...
<form action="<?php echo site_url('reports/download_parent_company_report') ?>" method="post" class="big" target="download_report_frame" name="report_options" id="report_options">
                        ...
                        <p class="clearfix" id="users">
                            <?php
                                ...
                                foreach ($childCompanies as $childCompany) {
                                    ?>
                                    <label style="text-align: left; display: block; width: auto; padding-right: 3px;" for="<?php echo $childCompany->companyId ?>">
                                        <input checked type="checkbox" name="company[<?php echo $childCompany->companyId ?>]" id="<?php echo $childCompany->companyId ?>" value ="<?php echo $childCompany->companyId ?>" class="companyCheck" style="margin-top: 0; margin-right: 4px; margin-left: 1px;">
                                        <span style="color:#444444;border-bottom:none"> <?php echo $childCompany->companyName ?></span>
                                    </label>
                                    <?php
                                }
                        ...
                        <div style="width: 100%;float:left;">
                            <?php
                                ...
                                foreach ($childAccounts as $acc) {
                                    ?>
                                    <div class="user_div" data-company-id="<?= $acc->getCompany()->getCompanyId();?>" style="width: 33%;float:left;">
                                    <label style="text-align: left; display: block; width: auto; padding-right: 3px;" for="<?php echo $acc->getAccountId() ?>">
                                        <input type="checkbox" data-company-id="<?= $acc->getCompany()->getCompanyId();?>" name="accounts[<?php echo $acc->getAccountId() ?>]" id="<?php echo $acc->getAccountId() ?>" class="userCheck" style="margin-top: 0; margin-right: 4px; margin-left: 1px;">
                                       <span class="tiptip" title="<?= $acc->getCompany()->getCompanyName();?>" style="color:#444444;border-bottom:none"> <?php echo $acc->getFullName() ?></span>
                                    </label>
                                    </div>
                                    <?php
                                }
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $(".download_csv").click(function () {
            ...
            $("#reportType").val($(this).attr('rel'));
            $("#report_options").submit();
            //swal('Please wait file downloading');
            swal({
                    title: 'Downloading..',
        ...
        function getstatus() {
            $.ajax({
                url: "<?php echo site_url('reports/getDownloadStatus') ?>",
                type: "POST",
                dataType: 'json',
                success: function(data) {
        ...
        $("#allUsers").click(function() {
           $('.userCheck').each(function() {
            ...
            //    console.log($(this).attr('data-company-id'));
        ...
        $(".companyCheck").click(function() {
            ...
            $('.userCheck').each(function() {
                if($(this).attr('data-company-id') == company_id){
            ...
            if(checked){
                ...
                $('.user_div[data-company-id="'+company_id+'"]').show();
            }else{
                $('.user_div[data-company-id="'+company_id+'"]').hide();
            }
        ...
        $('#download_report_frame').load(function(){
        //$(this).show();
...
<iframe src="<?php echo site_url('account/blank_page') ?>" onchange="content_finished_loading(this)" frameborder="0" name="download_report_frame"  id="download_report_frame" width="1" height="1" style="visibility: hidden; display: none;"></iframe>

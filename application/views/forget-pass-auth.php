                ...
                <form id="login-otp-form"  class="validate">
                    <table class="boxed-table" cellpadding="0" cellspacing="0" width="100%">
                        <tr>
                            <td>
                                <label>Email:</label>
                                <input type="hidden" name="email" id="email" class="text required email" value="<?= $this->session->userdata('recoveremail'); ?>">
                                <div id="email" style="margin-top:6px;"><?= $this->session->userdata('recoveremail'); ?></div>
                            </td>
                        </tr>
                        <tr class="even">
                            <td>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $("#AuthBtn").on("click", function (e) {
            ...
            var url = '<?php echo site_url('home/password_reset/'.$rcovery_code) ?>';

            var request = $.ajax({
                url:"/ajax/forget_pass_validate",
                type:"POST",
                data:{
                    "email":$("#email").val(),
                    "otp":$("#otp").val()

...
class CI_Redis2 {
	...
	public function __construct($params = array())
	{
		...
		else
		{
			...
			$config = array(
				'host' => $this->_ci->config->item('localhost'),
				'port' => $this->_ci->config->item('6379'),
				'password' => $this->_ci->config->item(''),
			);
		...
		if ( ! $this->_connection)
		{
			...
			log_message('error', 'Could not connect to Redis at ' . $config['host'] . ':' . $config['port']);
		}

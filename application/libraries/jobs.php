...
class Jobs {
	...
	public function __construct() {
		...
		log_message('debug', 'loading jobs lib');
	...
	private function perform(array $job) {
		...
		try {
			...
			// Execute job
			$this->_ci->mcurl->add_call('perform_job', 'post', site_url($url),$job['params']);
			$exec_job = $this->_ci->mcurl->execute();

			// Get data
			if (isset($exec_job['perform_job']['error'])) {
				// If there is any error trhow an exception
				throw new Exception('Job executions had an error:'.print_r($exec_job,true));
			...
			if(isset($job['params']['failed_job_id'])){
				$failed_job = $this->_ci->em->find('\models\FailedJob', $job['params']['failed_job_id']);
				$this->_ci->em->remove($failed_job);
				$this->_ci->em->flush();
            }
			...
			// Log fail perform
			log_message('error', 'Failed processing job <' . $job['description'] . '> in <' . $job['queue'] . '> queue: ' . $e->getMessage());
			...
			if(isset($job['params']['failed_job_id'])){
				...
				$this->_ci->em->persist($failed_job);
				$this->_ci->em->flush();
				if($new_resend==3){
					$this->create($_ENV['QUEUE_EMAIL'], 'jobs', 'failed_job_mail', ['controller'=>$job['controller'],'method'=>$job['method'],'params'=>$job['params']['failed_job_id']], 'test job');
				}
            }else{
				$failed_job = new FailedJob();
				$failed_job->setJobName($job['method']);
				$failed_job->setFailedAt(date('Y-m-d H:i:s'));
				$failed_job->setJobData(json_encode($job['params']));
				if(isset($job['params']['proposal_id'])){
					$failed_job->setJobType('proposal_campaign');
					$failed_job->setEntityId($job['params']['proposal_id']);
					$failed_job->setCampaignId($job['params']['pgsId']);
				}else if(isset($job['params']['clientId'])){
					$failed_job->setJobType('client_campaign');
					$failed_job->setEntityId($job['params']['clientId']);
					$failed_job->setCampaignId($job['params']['campaignId']);
				}
				...
				if($job['params']['uniqueArg'] =='proposal'){
					$campaign_email = $this->em->find('\models\ProposalGroupResendEmail', $job['params']['uniqueArgVal']);
				}else if($job['params']['uniqueArg'] =='client_resend_email_id'){
					$campaign_email = $this->em->find('\models\ClientGroupResendEmail', $job['params']['uniqueArgVal']);

                ...
                if (!$data['hasWarnings']) {
                    foreach($data['files'] as $key => $file) {
                        if (move_uploaded_file($file['tmp_name'], $file['file'])) {
							...
							$img = $imageManager->make(ROOTPATH.'/'.$file['file']);
							// Default orientation of portrait
							$orientation = 'portrait';
							// Only make landscape if not protrait or square
							if($img->width() > $img->height()){
								$orientation = 'landscape';
							}

							// Do a width check if landscape
    ...
    private function validate($file = null) {		
        if ($file == null) {
			...
			if ($this->field['input']['error'][$file['index']] > 0)
				return $this->codeToMessage(isset($this->field['error'])?$this->field['error'][$file['index']]:'', $file);
